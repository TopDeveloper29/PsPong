$logdate = Get-Date -Format "hh-mm-ss_yyyy-MM-dd"
...
{
Start-Transcript -Path "$PSScriptRoot`\$logdate`.log"
Remove-Item("temp.txt") -Force
$ms = Read-Host -Prompt "Clock Speed (ms)"
}
else
{
start powershell -ArgumentList ("-WindowStyle Maximized","$PSScriptRoot`\pong_debug.ps1")
...
$db = Read-Host -Prompt "Afficher les information de deboguage (0/1)"
$demo = Read-Host -Prompt "Mode Demo (0/1)"
...
{
    $ai = Read-Host -Prompt "Mode JCJ/IA (0/1)"
}

if($ai -eq 1)
{
$df = Read-Host -Prompt "Difuclter IA (1-3)"
...
Write-Host "        - Player 1 -
    skin : $p1_i
    ligne d'afichage : $p1_g
    X : $p1_x
    X on % : $z1
        - Player 2 -
    skin : $p2_i
    ligne d'afichage : $p2_g
    X : $p2_x
    X on %: $z2
        - Balle -
    skin : $b_i
    ligne d'affichage : $b_g
    X : $b_x
    X on % : $z3
    Y : $b_y
    Mode Y : $mode
...
}                                         ################## debug variable (echo before game display) #####################
if($ai -eq 0)
{
write-host "
   AI1 $pt1  ▐   AI2 $pt2   
